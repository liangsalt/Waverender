...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Export the waveform
  context.subscriptions.push(
    vscode.commands.registerCommand("waveformRender.saveAsPng", (savePath?: string) => {
      WaveformRenderPanel.saveAsPng(savePath);
    })
  );
  context.subscriptions.push(
  ...
  // Render and save as PNG (for use by other extensions)
  context.subscriptions.push(
    vscode.commands.registerCommand("waveformRender.renderAndSavePng", async (savePath?: string) => {
      // Close existing panel to ensure clean state
      ...
      // Render the waveform
      await vscode.commands.executeCommand("waveformRender.start");

      // Wait for the webview to render completely
      await new Promise(resolve => setTimeout(resolve, 800));

      // Save as PNG (with optional custom save path)
      await vscode.commands.executeCommand("waveformRender.saveAsPng", savePath);
    })
  );
}
...
class WaveformRenderPanel {
  ...
  private constructor(panel: vscode.WebviewPanel, extensionPath: string) {
    ...
    this._panel.webview.onDidReceiveMessage(
      async (message) => {
        if (message.command === 'savePngToFile') {
          try {
            const base64Data = message.pngDataUrl.replace(/^data:image\/png;base64,/, "");
            const buffer = Buffer.from(base64Data, 'base64');
            await vscode.workspace.fs.writeFile(vscode.Uri.file(message.savePath), buffer);
          } catch (err) {
  ...
  public static saveAsPng(savePath?: string) {
    if (WaveformRenderPanel.currentPanel) {
      WaveformRenderPanel.currentPanel._panel.webview.postMessage({
        command: "savePng",
        savePath: savePath,
  ...
  private _getHtmlForWebview(
    ...
    return `<!DOCTYPE html>
            ...
            window.addEventListener('message', async event => {
              ...
              if (command === 'savePng') {
                const savePath = event.data.savePath;
                ...
                img.onload = async function () {
                  ...
                  if (savePath) {
                    // Send PNG data back to extension for saving
                    vscode.postMessage({ command: 'savePngToFile', pngDataUrl: pngUrl, savePath: savePath });
                  } else {
